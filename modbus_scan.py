        ...
        if data_len == total_len - 6:
            if data[7] == chr(0x2b):
                ...
                for j in range(0,data_num):
                    ...
                    return_data += ' '
                    num = num + data1_len + 1
                return ("Unit ID:" + str(ord(data[6])), "Device: " + return_data)
            else:
                if data[7] == chr(0xab):
                    return ("Unit ID:" + str(ord(data[6])), "Device info error: " + _errors[ord(data[8])] if _errors.has_key(ord(data[8])) else "unknow error")
                else:
                    return ("Unit ID:" + str(ord(data[6])), "Device info error: unknow error")
        else:
            return ("Unit ID:" + str(ord(data[6])), "Device info error: unknow error")

if __name__ == '__main__':
    port = 502
    print modbus_scan('184.151.57.140', port)